...
// Updated: 2024-12-19 15:30:00 - Added debugging and file count display
...
export default function AssistantFiles({ files, referencedFiles }: AssistantFilesProps) {
  ...
  // Add debugging to see what files we're receiving
  ...
  return (
    <div className="w-full mt-4 bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
      ...
      >
        <span className="font-semibold text-gray-800 dark:text-gray-200">Assistant Files ({files.length})</span>
        <span className="text-xl text-gray-600 dark:text-gray-300">{isOpen ? '▲' : '▼'}</span>
      </button>
      {isOpen && (
        <div className="p-4">
          {/* Debug: Show all file names as text */}
          <div className="mb-4 p-2 bg-yellow-100 dark:bg-yellow-900 text-xs">
            <strong>Debug - All files:</strong> {files.map(f => f.name).join(', ')}
          </div>
